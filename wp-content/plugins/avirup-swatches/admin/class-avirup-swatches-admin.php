...
class Avs_Admin {
	...
	public function enqueue_styles() {
		...
		if( isset($_GET['page']) && $_GET['page'] == 'avirup-swatches'){
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/avirup-swatches-admin.css', array(), $this->version, 'all' );
	...
	public function avs_update_settings() {
		...
		if ( empty( $keys ) ) {
			wp_send_json_error( [ 'message' => __( 'No valid setting keys found.', 'avirup-swatches' ) ] );
		}
		...
		if ( count( $keys ) === $succeded ) {
			wp_send_json_success( [ 'message' => __( 'Settings saved successfully.', 'avirup-swatches' ) ] );
		}

		wp_send_json_error( [ 'message' => __( 'Failed to save settings.', 'avirup-swatches' ) ] );
	}
	...
	public function enqueue_scripts() {
		...
		if((isset($_GET['taxonomy']))){
			...
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/avirup-swatches-admin.js', array( 'jquery' ), $this->version, false );
			wp_localize_script(
				$this->plugin_name,
	...
	public function add_swatch_types( $fields ) {
		...
		if ( isset( $current_screen->base ) && 'product_page_product_attributes' === $current_screen->base ) {
			$fields = wp_parse_args(
				$fields,
				[
					'select' => esc_html__( 'Select', 'avirup-swatches' ),
					'color'  => esc_html__( 'Color', 'avirup-swatches' ),
					'image'  => esc_html__( 'Image', 'avirup-swatches' ),
				]
			);
		}
		return $fields;
	...
	public function admin_settings_menu(){
		add_menu_page(
			__('Avirup Swatches', 'avirup-swatches'),
			__('Avirup Swatches', 'avirup-swatches'),
			'manage_options',
			$this->plugin_name,
	...
	public function admin_settings_template(){
		if( isset($_POST['show_hide_variations']) ){
			update_option('show_hide_variations', sanitize_text_field($_POST['show_hide_variations']));
		}

		$db_value = get_option('show_hide_variations');

		?>
		<p>Show/Hide Swatches In Shop Page?</p>
		<form method="post">
			<div class="form-group">
				<div class="label">
					<label for="">Enable</label>
					<input type="radio" name="show_hide_variations" value="enable" <?php echo $db_value == 'enable'? 'checked' : ''; ?>>
				</div>
				<div class="label">
					<label for="">Disable</label>
					<input type="radio" name="show_hide_variations" value="disable" <?php echo $db_value == 'disable'? 'checked' : ''; ?>>
				</div>
			</div>
	...
	public function add_attribute_column( $columns ) {
		...
		$new_columns['preview'] = esc_html__( 'Preview', 'avirup-swatches' );
	...
	public function term_meta_fields( $type ) {
		...
		$fields = [
			'color' => [
				'label' => __( 'Color', 'avirup-swatches' ),
				'desc'  => __( 'Choose a color', 'avirup-swatches' ),
				'id'    => 'avs_product_attribute_color',
				'type'  => 'color',
			],
			'image' => [
				'label' => __( 'Image', 'avirup-swatches' ),
				'desc'  => __( 'Choose an image', 'avirup-swatches' ),
		...
		switch ( $field['type'] ) {
			case 'image':
				...
				<div class="meta-image-field-wrapper">
					<img class="avs-image-preview" height="60px" width="60px" src="<?php echo esc_url( $value ); ?>" alt="<?php esc_attr_e( 'Variation swatches image preview', 'avirup-swatches' ); ?>" style="<?php echo ( empty( $value ) ? 'display:none' : '' ); ?>" />
					<div class="button-wrapper">
						<input type="hidden" class="<?php echo esc_attr( $field['id'] ); ?>" name="avs_image" value="<?php echo esc_attr( $value ); ?>" />
						<button type="button" class="avs_upload_image_button button button-primary button-small"><?php esc_html_e( 'Upload image', 'avirup-swatches' ); ?></button>
						<button type="button" style="<?php echo ( empty( $value ) ? 'display:none' : '' ); ?>" class="avs_remove_image_button button button-small"><?php esc_html_e( 'Remove image', 'avirup-swatches' ); ?></button>
					</div>
				</div>
				<?php
				break;
